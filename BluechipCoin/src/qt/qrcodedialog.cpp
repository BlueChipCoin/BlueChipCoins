...
{
    QString ret = QString("BluechipCoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non BC unit input in lnReqAmount, we generate the URI with BKC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::BC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
